...
// 시즌기간("12~2 / 6~8" 등) → 해당 월 Set. 숫자 없으면 null(=시즌 미등록).
...
function isInSeasonNow(monthsSet) {
  if (monthsSet === null) return false; // 빈칸 = 시즌 미등록 = 시즌중 아님
...
export default function SalesForecast() {
  ...
  const applySnapshot = useCallback((s) => {
    // 캐시에 박힌 inSeason(옛 로직·이전 월 기준)을 현재 시즌기간 기준으로 교정
    const fixRow = (r) => ({ ...r, inSeason: isInSeasonNow(parseSeasonMonths(r.season?.period)) });
    setRows((s.rows || []).map(fixRow));
    setTop20((s.top20 || []).map(fixRow));
